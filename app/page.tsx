      ...
      <main className="flex-1 relative overflow-hidden">
          ...
          <motion.div custom={6} variants={fadeUp} initial="hidden" animate="visible"
            ...
            {[
              ...
              <div key={s.l} className="text-center">
                ...
                <div className="label-dt text-light-text-muted dark:text-text-secondary">{s.l}</div>
              </div>
            ))}
          </motion.div>
        </section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {why.map((w, i) => (
                <motion.div key={w.title}
                  ...
                  <div className="bg-light-surface dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-2xl p-5 h-full group hover:border-accent/30 transition-colors">
                    <div className="w-9 h-9 rounded-xl bg-light-gold/10 dark:bg-or/8 text-light-gold dark:text-or flex items-center justify-center mb-3 group-hover:bg-light-gold/20 dark:group-hover:bg-or/15 transition-colors">
                      <w.icon size={18} />
                    </div>
                    <h3 className="font-display font-semibold text-light-gold dark:text-or text-lg mb-1">{w.title}</h3>
                    <p className="text-sm text-light-text-muted dark:text-text-secondary leading-relaxed">{w.desc}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {features.map((f, i) => (
              <motion.div key={f.title}
                ...
                <div className="bg-light-surface dark:bg-dark-surface border border-light-border dark:border-dark-border rounded-2xl p-5 h-full group hover:border-accent/25 hover:shadow-[0_0_20px_rgba(0,212,255,0.06)] transition-all">
                  ...
                  <div className="flex items-center gap-2 mb-1.5">
                    <h3 className="font-display font-semibold text-light-gold dark:text-or text-base">{f.title}</h3>
                  </div>
                  <p className="text-xs text-light-text-muted dark:text-text-secondary leading-relaxed mb-2">{f.desc}</p>
        ...
        <section className="relative z-10 max-w-5xl mx-auto px-6 py-24">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            ...
            <p className="label-dt text-light-text-muted dark:text-text-secondary mb-3">Témoignages</p>
            <h2 className="font-display text-3xl font-light text-light-text dark:text-white">Ils nous font confiance</h2>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {testimonials.map((t, i) => (
              <motion.div key={t.name}
                ...
                <div className="bg-light-surface dark:bg-dark-surface border border-light-border dark:border-dark-border rounded-2xl p-5 h-full">
                  <div className="flex gap-0.5 mb-3">
                    {Array.from({ length: t.stars }).map((_, s) => (
                      <Star key={s} size={13} className="text-light-gold dark:text-or fill-light-gold dark:fill-or" />
                    ))}
                  </div>
                  <p className="text-sm text-light-text dark:text-dark-text leading-relaxed mb-4 italic">"{t.text}"</p>
                  <div>
                    <p className="text-sm font-semibold text-light-text dark:text-white font-sans">{t.name}</p>
                    <p className="text-xs text-light-text-muted dark:text-text-secondary">{t.role}</p>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <section className="relative z-10 px-6 pb-24">
          <motion.div initial={{ opacity: 0, scale: 0.97 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
            className="max-w-2xl mx-auto text-center border border-light-gold/20 dark:border-or/20 bg-light-gold/5 dark:bg-or/3 rounded-3xl px-8 py-14">
            <div className="w-14 h-14 rounded-2xl bg-light-gold/10 dark:bg-or/10 text-light-gold dark:text-or flex items-center justify-center mx-auto mb-5">
              <Zap size={26} />
            </div>
