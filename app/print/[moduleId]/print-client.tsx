...
function SlideCard({ slide, index, total }: { slide: SlideWithContent; index: number; total: number }) {
  ...
  if (slide.type === 'quote') {
    ...
    const bgMap: Record<string, string> = {
      cyan: 'linear-gradient(135deg,#0A2A35,#0C0C14)',
      purple: 'linear-gradient(135deg,#1A0A35,#0C0C14)',
      dark: '#080810',
      amber: 'linear-gradient(135deg,#2A1A00,#0C0C14)',
    }
    return (
...
export default function PrintClient({ moduleTitle, slides }: Props) {
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: CSS }} />
      <PrintButton title={moduleTitle} count={slides.length} />
