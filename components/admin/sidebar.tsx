...
export function AdminSidebar({ userEmail, userRole }: AdminSidebarProps) {
  ...
  return (
    <aside className="w-64 h-screen flex flex-col bg-light-surface dark:bg-dark-bg border-r border-light-border dark:border-light-border dark:border-dark-border sticky top-0 flex-shrink-0 transition-colors duration-300">
      {/* Logo */}
      <div className="px-5 py-5 border-b border-light-border dark:border-light-border dark:border-dark-border">
        <Link
          href="/"
          className="flex items-center gap-2 font-display font-bold text-lg hover:opacity-85 transition-opacity"
        >
          <Zap className="text-accent" size={18} />
          <span className="text-light-text dark:text-white">Deck</span><span className="text-or dark:text-or">Train</span>
        </Link>
        <p className="text-[10px] text-text-secondary mt-0.5 ml-[26px] label-dt">
          Administration
        </p>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
              ...
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ...
                    : 'text-text-secondary hover:text-dark-text hover:bg-white/4'
                )}
              >
        ...
        <Link
          href="/present"
          target="_blank"
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-text-secondary hover:bg-accent/6 hover:text-accent transition-all duration-200"
        >
      ...
      <div className="px-3 py-4 border-t border-light-border dark:border-dark-border space-y-2">
        ...
        <div className="flex items-center gap-3 px-3 py-1">
          <span className="text-[10px] text-text-secondary flex-1 label-dt">Thème</span>
          <ThemeToggle />
        </div>

        {/* Email + rôle */}
        <div className="px-3 py-1">
          <p className="text-xs text-text-secondary truncate">{userEmail ?? 'admin'}</p>
          {userRole && (
        ...
        <button
          ...
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium text-text-secondary hover:bg-red-500/10 hover:text-red-400 transition-all duration-200"
        >
          <LogOut size={16} />
          Se déconnecter
        </button>
